    ...
    def convert_book(self, input_path, output_format):
        ...
        try:
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"Conversion failed: {process.stderr}"
                )
...
class BookConverterGUI:
    ...
    def _create_widgets(self):
        ...
        ttk.Entry(select_frame, textvariable=self.file_path, width=50).pack(side=tk.LEFT, padx=5)
        ttk.Button(select_frame, text="Browse", command=self._browse_file).pack(side=tk.LEFT)
    ...
    def _convert(self):
        ...
        try:
            ...
            self.status_var.set(f"Conversion successful!\nOutput saved to: {output_path}")
